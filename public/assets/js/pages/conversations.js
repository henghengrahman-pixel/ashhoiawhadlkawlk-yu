...
const state={cursor:null,hasMore:false,lane:'MY_CHAT',filter:'ALL',query:'',listActive:null,listVersion:0,detailAbort:null,detailVersion:0,listTimer:null,messageTimer:null,messageBusy:false,messageAfterId:0,pollDelay:4000,imageFile:null,shortcutIndex:0,shortcuts:[],shortcutAbort:null,shortcutVersion:0,messageCursor:null,loadingOlder:false,sendBusy:false,lastSyncAt:0};
...
function isClosedChatError(e){return Number(e?.status)===410||String(e?.code||'')==='LIVECHAT_CHAT_INACTIVE'||/chat[_\s-]*(?:is[_\s-]*)?(?:not[_\s-]*active|inactive)/i.test(String(e?.message||''))}
function removeClosedConversation(id,message='Percakapan sudah ditutup di LiveChat.'){
  if(selected()===String(id)){store.remove(String(id));renderRows();clearConversation(message)}else{store.remove(String(id));renderRows()}
  fetchList({silent:true,reason:'closed'}).catch(()=>{});
}
...
function chatRowHtml(c){const mode=modeOf(c),lane=String(c.lc_lane||'OTHER').toUpperCase();return `<div class="avatar">${esc((c.customer_name||'?').slice(0,1).toUpperCase())}</div><div class="chat-copy"><div class="chat-title"><b>${esc(c.customer_name||c.chat_id)}</b><time>${esc(fmtTime(c.last_message_at||c.last_event_at))}</time></div><div class="chat-last">${esc(c.last_message||'Belum ada pesan')}</div><div class="chat-row-bottom"><span class="mode-text ${mode==='HUMAN'?'human':''}">${mode}</span><span class="lane-mini">${esc(lane.replace('_',' '))}</span>${c.needs_reply?'<span class="needs-reply" title="Menunggu balasan"></span>':''}</div></div>`}
function renderRows(){
  ...
  $('#inboxCount').textContent=`${items.length}${state.hasMore?'+':''} ${state.lane==='MY_CHAT'?'my chats':state.lane.toLowerCase()}`;$('#keyboardNavCount').textContent=`${items.length}${state.hasMore?'+':''}`;$('#loadMoreChats').classList.toggle('hidden',!state.hasMore);
}
async function fetchList({append=false,silent=false,reason='user'}={}){
  ...
  const params=new URLSearchParams({limit:'50',filter:state.filter,lane:state.lane});if(append&&requestCursor)params.set('cursor',requestCursor);if(state.query)params.set('q',state.query);
  try{
    ...
    state.hasMore=Boolean(data.page?.hasMore);state.cursor=data.page?.nextCursor||null;
    const counts=data.laneCounts||{};$('#laneMyCount').textContent=String(counts.MY_CHAT||0);$('#laneQueuedCount').textContent=String(counts.QUEUED||0);$('#laneSupervisedCount').textContent=String(counts.SUPERVISED||0);$('#laneTrafficCount').textContent=String(counts.TRAFFIC||0);
    append?store.merge(data.items||[]):store.replace(data.items||[]);renderRows();
...
async function pollMessages(){const id=selected();if(!id||state.messageBusy||document.hidden)return;state.messageBusy=true;try{const r=await api(`/api/conversations/${encodeURIComponent(id)}/messages?afterId=${state.messageAfterId}&limit=50`,{timeout:8000});if(selected()!==id)return;if(r.conversation?.status==='closed'||r.conversation?.visible_in_inbox===false){store.remove(id);renderRows();clearConversation('Percakapan telah ditutup.');return}const items=r.items||[];if(items.length){state.messageAfterId=Number(items.at(-1).id||state.messageAfterId);renderMessages(items,{append:true});fetchList({silent:true,reason:'poll'})}if(r.conversation){setMode(modeOf(r.conversation),String(r.conversation.status||'ACTIVE').toUpperCase(),r.conversation.lc_lane);renderSide(r.conversation,r.state||{})}setConnection('LIVE')}catch(e){if(e.name!=='AbortError')setConnection(navigator.onLine?'RECONNECTING':'OFFLINE')}finally{state.messageBusy=false}}
...
$('#claimAiBtn').onclick=()=>action($('#claimAiBtn'),'Claiming...',async()=>{const id=selected();if(!id)return;await api(`/api/conversations/${encodeURIComponent(id)}/claim`,{method:'POST',body:JSON.stringify({mode:'AI'}),timeout:15000});if(selected()===id)await openChat(id);await fetchList({silent:true,reason:'claim'});toast('Chat sekarang ditangani AI dari dashboard')});
...
document.querySelectorAll('[data-lane]').forEach(b=>b.onclick=()=>{
  const lane=String(b.dataset.lane||'MY_CHAT').toUpperCase();if(lane===state.lane)return;
  document.querySelectorAll('[data-lane]').forEach(x=>x.classList.toggle('active',x===b));state.lane=lane;state.cursor=null;state.hasMore=false;store.replace([]);renderRows();clearConversation(`Pilih percakapan dari ${lane.replace('_',' ')}.`);fetchList({reason:'lane'});
});
