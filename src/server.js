...
import { startPoller, stopPoller, pollerStatus, syncOnce, forgetChat, syncLaneChatForOperator } from './poller.js';
import { enqueueLiveChatWebhook, startLiveChatIngressWorker, stopLiveChatIngressWorker, liveChatIngressStatus } from './livechat-ingress.js';
...
async function closeConversationFromInactiveProvider(chatId,error,reason='LIVECHAT_CONFIRMED_INACTIVE'){
  const inactive=lc?.isChatInactiveError?.(error) || error?.code==='LIVECHAT_CHAT_INACTIVE';
  if(!inactive)return false;
  await markConversationEnded(chatId,{reason}).catch(()=>{});
  forgetChat(chatId);
  return true;
}
const learningWorker=createLearningWorker({
  ...
  withLock:withLearningAdvisoryLock,isBusy:()=>Boolean(pollerStatus().inFlight)
});
...
app.get('/api/ops/health',requireAdmin,async(req,res)=>{
  try{
    const items=await getIntegrationHealth();
    const errors=items.filter(x=>String(x?.status||'').toUpperCase()==='ERROR');
    res.json({ok:errors.length===0,errors:errors.length,items});
  }catch(e){
    res.status(503).json({ok:false,errors:1,error:'HEALTH_READ_FAILED',items:[]});
  }
});
...
async function refreshDashboardLaneChat(chatId){
  const st=await getConversationState(chatId).catch(()=>null);
  if(!st || String(st.status||'').toLowerCase()==='closed' || st.lc_active===false) return {closed:true};
  const lane=String(st.lc_lane||'OTHER').toUpperCase();
  if(lane==='MY_CHAT') return {ok:true,lane,skipped:'already_my_chat'};
  return syncLaneChatForOperator(lc,chatId,{id:chatId});
}

async function claimDashboardChat(chatId){
  const id=String(chatId||'').trim(); if(!id){const e=new Error('LIVECHAT_CHAT_ID_REQUIRED');e.status=400;throw e;}
  const st=await getConversationState(id).catch(()=>null);
  if(st && (String(st.status||'').toLowerCase()==='closed' || st.lc_active===false)){const e=new Error('LIVECHAT_CHAT_INACTIVE');e.status=410;throw e;}
  if(String(st?.lc_lane||'').toUpperCase()==='MY_CHAT') return {ok:true,lane:'MY_CHAT',alreadyClaimed:true};
  const claimed=await lc.claimChat(id,{ensureMembership:true});
  const refreshed=await syncLaneChatForOperator(lc,id,claimed.chat||{id});
  // claimChat succeeded as this requester, so the dashboard is allowed to treat the chat
  // as MY_CHAT immediately even if list_chats is a few seconds behind.
  await pool.query(`UPDATE conversations SET lc_lane='MY_CHAT',visible_in_inbox=true,lc_active=true,updated_at=now() WHERE chat_id=$1 AND status='active'`,[id]);
  return {ok:true,lane:'MY_CHAT',providerLane:claimed.lane||refreshed?.lane||null,membership:claimed.membership||null};
}
...
app.get('/api/conversations',requireAdmin,async(req,res)=>{
  ...
  const requestedLane=String(req.query.lane||'MY_CHAT').trim().toUpperCase();
  const lane=['MY_CHAT','QUEUED','SUPERVISED','TRAFFIC','OTHER'].includes(requestedLane)?requestedLane:'MY_CHAT';
  let cursor=null;
  ...
  const params=[q,filter,limit+1,cursorRank,cursorTime,cursorId,lane];
  const [r,countRows]=await Promise.all([
    pool.query(`SELECT c.chat_id,c.customer_name,c.customer_email,c.status,c.handling_mode,c.takeover_reason,c.takeover_at,c.last_event_at,c.last_member_event_at,c.last_ai_event_at,c.workflow_type,c.workflow_state,c.member_typing,c.member_typing_updated_at,c.created_at,c.updated_at,c.lc_inbox_rank,c.lc_lane,
    ...
    WHERE c.visible_in_inbox=true AND c.status='active' AND c.lc_active IS DISTINCT FROM false
      ...
      AND ($7='TRAFFIC' OR upper(trim(coalesce(c.lc_lane,'OTHER')))=$7)
    ...
    LIMIT $3`,params),
    pool.query(`SELECT upper(trim(coalesce(lc_lane,'OTHER'))) AS lc_lane,count(*)::int AS count FROM conversations WHERE visible_in_inbox=true AND status='active' AND lc_active IS DISTINCT FROM false GROUP BY upper(trim(coalesce(lc_lane,'OTHER')))`)
  ]);
  ...
  const laneCounts={MY_CHAT:0,QUEUED:0,SUPERVISED:0,OTHER:0,TRAFFIC:0};
  for(const row of countRows.rows||[]){const key=String(row.lc_lane||'OTHER').trim().toUpperCase();if(Object.prototype.hasOwnProperty.call(laneCounts,key)) laneCounts[key]=Number(row.count||0);laneCounts.TRAFFIC+=Number(row.count||0);}
  res.json({ok:true,items,lane,laneCounts,traffic:{internal:true,chatInventoryOnly:true,browsingOnlyVisitors:false},page:{limit,nextCursor,hasMore}});
});
app.get('/api/conversations/:id',requireAdmin,async(req,res)=>{
  ...
  const pre=await getConversationState(req.params.id).catch(()=>null);
  if(pre && String(pre.lc_lane||'MY_CHAT').toUpperCase()!=='MY_CHAT' && String(pre.status||'active').toLowerCase()!=='closed'){
    await refreshDashboardLaneChat(req.params.id).catch(async e=>{await logError('conversation','LANE_DETAIL_REFRESH_FAILED',e.message,{chatId:req.params.id,lane:pre.lc_lane}).catch(()=>{});});
  }
  const metaPromise=pool.query(`SELECT c.chat_id,c.customer_name,c.customer_email,c.status,c.handling_mode,c.handling_state,c.takeover_reason,c.takeover_at,c.last_event_at,c.last_member_event_at,c.last_ai_event_at,c.workflow_type,c.workflow_state,c.visible_in_inbox,c.lc_lane,
...
app.get('/api/conversations/:id/messages',requireAdmin,async(req,res)=>{
  ...
  const pre=await getConversationState(req.params.id).catch(()=>null);
  if(pre && String(pre.lc_lane||'MY_CHAT').toUpperCase()!=='MY_CHAT' && String(pre.status||'active').toLowerCase()!=='closed') await refreshDashboardLaneChat(req.params.id).catch(()=>{});
  const [meta,msg,state]=await Promise.all([
    pool.query(`SELECT c.chat_id,c.customer_name,c.customer_email,c.status,c.handling_mode,c.handling_state,c.takeover_reason,c.takeover_at,c.last_event_at,c.last_member_event_at,c.last_ai_event_at,c.workflow_type,c.workflow_state,c.visible_in_inbox,c.lc_lane,CASE WHEN (SELECT m.sender_type FROM messages m WHERE m.chat_id=c.chat_id ORDER BY m.id DESC LIMIT 1)='customer' THEN true ELSE false END AS needs_reply FROM conversations c WHERE c.chat_id=$1 LIMIT 1`,[req.params.id]),
...
app.post('/api/conversations/:id/takeover',requireAdmin,async(req,res)=>{try{const chatId=String(req.params.id);const out=await withChatLock(chatId,async()=>{const st=await getConversationState(chatId).catch(()=>null);let claimed=null;if(String(st?.lc_lane||'MY_CHAT').toUpperCase()!=='MY_CHAT')claimed=await claimDashboardChat(chatId);await setHumanTakeover(chatId,'dashboard_takeover');return {ok:true,mode:'HUMAN',claimed};});res.json(out);}catch(e){const providerLimit=String(e?.code||'')==='LIVECHAT_PUBLIC_AGENT_LIMIT'||/PUBLIC_AGENT_LIMIT/i.test(String(e?.message||''));res.status(providerLimit?409:(e.status||502)).json({ok:false,error:e.message,providerLimit});}});
app.post('/api/conversations/:id/enable-ai',requireAdmin,async(req,res)=>{
  try{
    const chatId=String(req.params.id);let claimed=null;
    await withChatLock(chatId,async()=>{const st=await getConversationState(chatId).catch(()=>null);if(String(st?.lc_lane||'MY_CHAT').toUpperCase()!=='MY_CHAT')claimed=await claimDashboardChat(chatId);await clearHumanTakeover(chatId);});
    const resumed=await resumeConversationAfterHumanTakeover({chatId,livechat:lc});
    res.json({ok:true,mode:'AI',claimed,resumed});
  }catch(e){
    await logError('conversation','ENABLE_AI_RESUME_FAILED',e.message,{chatId:req.params.id});
    const providerLimit=String(e?.code||'')==='LIVECHAT_PUBLIC_AGENT_LIMIT'||/PUBLIC_AGENT_LIMIT/i.test(String(e?.message||''));
    res.status(providerLimit?409:(e.status||502)).json({ok:false,error:e.message,providerLimit});
  }
});
app.post('/api/conversations/:id/claim',requireAdmin,async(req,res)=>{
  try{
    const chatId=String(req.params.id),mode=String(req.body?.mode||'AI').toUpperCase();
    const claimed=await withChatLock(chatId,()=>claimDashboardChat(chatId));
    if(mode==='HUMAN') await setHumanTakeover(chatId,'dashboard_claim'); else await clearHumanTakeover(chatId);
    const resumed=mode==='AI'?await resumeConversationAfterHumanTakeover({chatId,livechat:lc}):null;
    res.json({ok:true,mode:mode==='HUMAN'?'HUMAN':'AI',claimed,resumed});
  }catch(e){const providerLimit=String(e?.code||'')==='LIVECHAT_PUBLIC_AGENT_LIMIT'||/PUBLIC_AGENT_LIMIT/i.test(String(e?.message||''));res.status(providerLimit?409:(e.status||502)).json({ok:false,error:e.message,providerLimit});}
});
...
app.post('/api/conversations/:id/send',requireAdmin,async(req,res)=>{const text=String(req.body?.text||'').trim();if(!text)return res.status(400).json({ok:false,error:'EMPTY_TEXT'});try{const out=await withChatLock(req.params.id,async()=>{if(!(await isHumanTakeover(req.params.id))) { const er=new Error('HUMAN_TAKEOVER_REQUIRED'); er.status=409; throw er; } const sent=await lc.sendMessage(req.params.id,text);const eventId=sent?.event_id||sent?.id||null;if(eventId){await insertMessage({chatId:req.params.id,eventId:String(eventId),senderType:'agent',authorId:'admin',text,normalizedText:normalizeText(text),intent:detectIntent(text),createdAt:new Date().toISOString()});await captureHumanReplyLearning({chatId:req.params.id,eventId:String(eventId),responseText:text}).catch(()=>{});}return {ok:true,sent};});res.json(out);}catch(e){if(await closeConversationFromInactiveProvider(req.params.id,e,'MANUAL_SEND_CONFIRMED_INACTIVE'))return res.status(410).json({ok:false,error:'LIVECHAT_CHAT_INACTIVE',closed:true});res.status(e.status||502).json({ok:false,error:e.message});}});
app.post('/api/conversations/:id/image',requireAdmin,async(req,res)=>{
  try{
    ...
    res.json(out);
  }catch(e){await logError('manual_image','SEND_FAILED',e.message,{chatId:req.params.id});if(await closeConversationFromInactiveProvider(req.params.id,e,'MANUAL_IMAGE_CONFIRMED_INACTIVE'))return res.status(410).json({ok:false,error:'LIVECHAT_CHAT_INACTIVE',closed:true});res.status(e.status||502).json({ok:false,error:e.message});}
});
...
app.post('/webhooks/livechat',async(req,res)=>{
  try{
    ...
    // ACK only after the event has been durably persisted in PostgreSQL. AI/provider work
    // is intentionally asynchronous so webhook delivery never waits on OpenAI/LiveChat.
    const accepted=await enqueueLiveChatWebhook(req.body||{});
    return res.status(202).json({ok:true,...accepted});
  }catch(e){
    await logError('webhook','WEBHOOK_FAILED',e.message);
    return res.status(503).json({ok:false,error:'WEBHOOK_PERSIST_FAILED'});
  }
});
...
async function boot(){
  try{
    ...
    await withPostgresStartupRetry('health-reset',()=>resetIntegrationHealth());
    ...
    const shutdown=async(signal)=>{
      ...
      const realtimeStops=[stopLiveChatIngressWorker({waitMs:7000}),stopPoller({waitMs:5000}),stopCannedSync({waitMs:5000}),stopHumanBridge({waitMs:7000})];
