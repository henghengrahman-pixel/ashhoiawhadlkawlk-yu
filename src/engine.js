...
import { extractUserIdFromText, extractRequestedUserIdFromText, extractInlineBusinessUserIdFromText, extractKnownUserIdFromMessages } from './user-id.js';
...
import { runConversationAgent, validateAgentDecision, agentDecisionToLegacy, inferSessionCaseFromHistory } from './ai-agent.js';

const ai = new OpenAIClient();
const processingScope = new AsyncLocalStorage();
const AGENT_NON_CASE_INTENTS=new Set(['','GENERAL','GREETING','UNKNOWN','NONE','NEW','ACKNOWLEDGEMENT','GRATITUDE','ABUSIVE','COMPLAINT']);
...
function compactRulesForPrompt(rows=[],query='',intent='GENERAL',limit=20){
  const list=Array.isArray(rows)?rows:[];
  const q=normalizeText(query||'');
  const qTokens=[...new Set(q.split(/\s+/).filter(x=>x.length>=3))].slice(0,40);
  const it=String(intent||'GENERAL').toUpperCase();
  const critical=/\b(?:human_active|no_reply|member\s*id|user\s*id|bukti|proof|ticket|workflow|state|session|jangan\s+(?:meminta|tanya|ulang|mengulang)|prompt\s*injection|instruksi\s*internal|source\s*of\s*truth)\b/i;
  const scored=list.map((row,index)=>{
    const cat=String(row?.category||'').toUpperCase();
    const content=String(row?.content||'');
    const hay=normalizeText(`${row?.category||''} ${row?.rule_type||''} ${content}`);
    let lexical=0;
    for(const token of qTokens) if(hay.includes(token)) lexical+=1;
    const specific=cat===it?8:(cat!=='GLOBAL'?4:0);
    const must=critical.test(content)?6:0;
    return {row,index,score:specific+must+lexical,critical:Boolean(must),specific:cat!=='GLOBAL'};
  }).sort((a,b)=>b.score-a.score||a.index-b.index);
  const cap=Math.max(8,Math.min(40,Number(limit)||20));
  const chosen=[]; const seen=new Set();
  const add=x=>{if(!x||chosen.length>=cap)return;const key=String(x.row?.id??`${x.row?.category}:${x.index}`);if(seen.has(key))return;seen.add(key);chosen.push(x);};
  scored.filter(x=>x.critical).slice(0,Math.min(8,cap)).forEach(add);
  scored.filter(x=>x.specific).forEach(add);
  scored.forEach(add);
  return chosen.sort((a,b)=>a.index-b.index).map(x=>x.row);
}
...
function extractUserId(rows){
  // Resolve known ID from the entire CURRENT SESSION, including explicit notation,
  // inline business messages, and standalone answers after BOT/CS asked for ID.
  return extractKnownUserIdFromMessages(rows);
}
...
function currentCaseUserId(text='',wf=null,ctx=[],intent=''){
  const explicit=extractUserIdFromText(text);
  if(explicit) return explicit;

  const inline=extractInlineBusinessUserIdFromText(text,{intent:intent||workflowIntentFromType(wf?.workflow_type)});
  if(inline) return inline;

  const prev=String(wf?.workflow_data?.userId||'').trim();
  if(prev) return prev;

  // Reuse a field already supplied anywhere in the CURRENT SESSION.
  const known=extractUserId(ctx);
  if(known) return known;

  // Loose/standalone ID inference is only permitted after the bot explicitly asked for ID.
...
async function resolveContextualIntent(chatId,currentIntent,text,attachments=[]){
  ...
  const explicitOperational=new Set([
    ...
    'LINK_PROBLEM','LOGIN_PROBLEM','GAME_PROBLEM','GENERAL_DISTURBANCE','LOSS_COMPLAINT','ACCOUNT_CLOSE_REQUEST','TRANSACTION_AMBIGUOUS'
  ]);
  ...
  const sessionCase=inferSessionCaseFromHistory(rows,{currentText:text,currentIntent:current});
  // Anger/profanity is sentiment, not a replacement for the active DP/WD/reset/bonus case.
  // Explicit LOSS_COMPLAINT remains a real topic switch; generic/abusive short replies inherit
  // the latest operational case from the same session.
  if(['GENERAL','GREETING','UNKNOWN','ABUSIVE','COMPLAINT'].includes(current) && sessionCase) return sessionCase;
  if(['DEPOSIT_PROBLEM','DEPOSIT_CANCEL','WITHDRAW_PROBLEM','WITHDRAW_REQUEST','FORGOT_PASSWORD','BONUS_REQUEST','BONUS_DAILY','ACCOUNT_CHANGE_REQUEST','PAYOUT_NOT_RECEIVED','BANK_ACCOUNT_LIMIT','REGISTER_PROBLEM','LINK_PROBLEM','LOGIN_PROBLEM','GAME_PROBLEM','GENERAL_DISTURBANCE','LOSS_COMPLAINT','ACCOUNT_CLOSE_REQUEST','TRANSACTION_AMBIGUOUS'].includes(current)) return current;
...
async function maybeHandleOperationalFlow({chatId,eventId,text,intent,livechat}){
  ...
  const explicitTopicIntents=new Set([
    ...
    'LINK_PROBLEM','LOGIN_PROBLEM','REGISTER_PROBLEM','GENERAL_DISTURBANCE','LOSS_COMPLAINT','ACCOUNT_CLOSE_REQUEST','TRANSACTION_AMBIGUOUS'
  ]);
...
minta batalkan fom deposit nya`;
  ...
  if(wf?.workflow_type==='PROOF_REVIEW' || (latestProof(ctx) && ['GENERAL','GREETING','UNKNOWN'].includes(effective))){
    ...
    const userId=currentCaseUserId(text,wf,ctx,effective) || prev.userId || '';
...
// Deposit complaint: member may send ID and proof separately. Persist/merge both, then send one verification ticket to Telegram CS.
  if(['DEPOSIT_PROBLEM','DEPOSIT_REQUEST'].includes(effective) || wf?.workflow_type==='DEPOSIT_VERIFY'){
    ...
    const userId=currentCaseUserId(text,wf,ctx,effective) || (workflowAskedForId ? extractStandaloneRequestedUserId(ctx) : '');
    const proof=latestProof(ctx);
...
${data.forgotUserId?'Member lupa User ID dan password. Mohon cari akun dari data rekening/e-wallet terdaftar lalu kirim USER ID + PASSWORD + LINK.':'reset password ko'}`,requireTelegramDelivery:true});
  ...
  if(wf?.workflow_type==='WD_CHECK' && wf?.workflow_state==='WAITING_ID'){
    // Legacy regression compatibility marker: extractUserIdFromText(text) || extractRequestedUserIdFromText(text)
    // Legacy regression marker: holding:WD_WAITING_STAFF_REPLY
    const uid=currentCaseUserId(text,wf,ctx,'WITHDRAW_PROBLEM');
  ...
  if(['WITHDRAW_PROBLEM','WITHDRAW_REQUEST'].includes(effective)){
    ...
    const uid=currentCaseUserId(text,wf,ctx,effective);
    if(!uid){
...
Silakan cek kelayakan claim member.`;
  ...
  if(effective.includes('BONUS')){
    ...
    if(isFreebet50(text,bonusType)){
      const freebetUid=currentCaseUserId(text,wf,ctx,effective);
    ...
    const uid=currentCaseUserId(text,wf,ctx,effective) || (wf?.workflow_type==='BONUS_CLAIM' ? extractStandaloneRequestedUserId(ctx) : '');
...
`:''}${all.slice(-600)}`,requireTelegramDelivery:true});
  ...
  if(['LOGIN_PROBLEM','LINK_PROBLEM','REGISTER_PROBLEM','GENERAL_DISTURBANCE'].includes(effective) || wf?.workflow_type==='ACCESS_CHECK'){
    const prev=wf?.workflow_data||{};
    const uid=currentCaseUserId(text,wf,ctx,effective) || prev.userId || '';
    const proof=latestProof(ctx);
...
function retryableStatusFromError(err){
  const own=Number(err?.status||String(err?.message||'').match(/(?:OPENAI_|HTTP_|STATUS[_ :])(\d{3})/i)?.[1]||0);
  if(own)return own;
  return err?.cause ? retryableStatusFromError(err.cause) : 0;
}
function isRetryableTurnError(err){
  if(!err)return false;
  if(err.retryable===true)return true;
  const code=String(err.code||err.cause?.code||'').toUpperCase();
  const status=retryableStatusFromError(err);
  if([408,409,425,429,500,502,503,504].includes(status) || status>=500)return true;
  if(err.name==='AbortError' || /(?:ECONNRESET|ECONNREFUSED|ETIMEDOUT|EAI_AGAIN|ENETUNREACH|NETWORK|FETCH FAILED|SOCKET)/i.test(`${code} ${err.message||''}`))return true;
  return err.cause ? isRetryableTurnError(err.cause) : false;
}

async function sendAndStore(livechat,chatId,text,intent,senderType='ai'){
  if(!(await db.conversationCanReply(chatId,{requireMyChat:senderType==='ai'}))) throw new Error(senderType==='ai'?'CHAT_NOT_IN_AI_LANE':'CHAT_CLOSED_NO_REPLY');
  ...
  const scope=processingScope.getStore();
  const sourceEventId=String(scope?.eventId||'').trim()||null;
  let idempotencyKey=null,customId=null,outboundAction=null;
  if(sourceEventId){
    const current=await db.getConversationState(chatId).catch(()=>null);
    const sessionIdentity=String(current?.session_id||current?.session_key||scope?.sessionId||chatId);
    const ordinal=Number(scope.sendOrdinal||0);
    scope.sendOrdinal=ordinal+1;
    outboundAction=`SEND_MESSAGE:${String(senderType||'ai').toUpperCase()}:${String(intent||'GENERAL').toUpperCase()}:${ordinal}`;
    idempotencyKey=crypto.createHash('sha256').update(`${sessionIdentity}|${sourceEventId}|${outboundAction}`).digest('hex');
    customId=`lcai_${idempotencyKey.slice(0,40)}`;
    const prepared=await db.prepareOutbound(chatId,{text,eventId:sourceEventId,idempotencyKey,action:outboundAction});
    if(String(prepared?.status||'').toLowerCase()==='sent'){
      await db.appendCaseAudit({chatId,eventType:'OUTBOUND_IDEMPOTENCY_HIT',intent,payload:{sourceEventId,action:outboundAction}}).catch(()=>{});
      return {suppressed:true,reason:'outbound_idempotency_hit',event_id:prepared?.row?.livechat_event_id||null};
    }
  } else if(senderType==='ai' && String(intent||'').toUpperCase()!=='GREETING' && await db.shouldSuppressOutbound(chatId,text,120)){
    // Text-hash suppression is only a compatibility guard for outbound work that has no
    // provider input event. Valid customer events are never deduplicated by text.
  ...
  try{
    ...
    for(let attempt=1;attempt<=maxAttempts;attempt++){
      try{
        sent=await livechat.sendMessage(chatId,text,customId?{customId}:undefined);
        lastSendError=null;
        break;
      }catch(err){
        lastSendError=err;
    ...
    if(idempotencyKey) await db.completeOutbound(idempotencyKey,{eventId:sentEventId,text});
    else await db.saveOutbound(chatId,text,sentEventId);
    ...
    await db.appendCaseAudit({chatId,eventType:'MEMBER_REPLY_SENT',intent,payload:{senderType,sourceEventId,idempotencyKey:idempotencyKey?`${idempotencyKey.slice(0,12)}…`:null,textPreview:String(text).slice(0,240)}}).catch(()=>{});
    return sent;
  }catch(err){
    if(idempotencyKey) await db.failOutbound(idempotencyKey,err?.message||String(err)).catch(()=>{});
    const inactive=Boolean(livechat?.isChatInactiveError?.(err) || err?.code==='LIVECHAT_CHAT_INACTIVE');
    const publicAgentLimit=String(err?.code||'')==='LIVECHAT_PUBLIC_AGENT_LIMIT' || /PUBLIC_AGENT_LIMIT|Public agents in chat limit reached/i.test(String(err?.message||''));
    if(inactive) await db.markConversationEnded(chatId,{reason:'SEND_CONFIRMED_INACTIVE'}).catch(()=>{});
    else if(publicAgentLimit){
      await db.setConversationLane(chatId,'SUPERVISED').catch(()=>{});
      err.retryable=false;
      await db.setIntegrationHealth('livechat_membership',{status:'WARNING',latencyMs:Date.now()-started,error:String(err?.message||err),meta:{lastAction:'sendMessage',chatId,lane:'SUPERVISED'}}).catch(()=>{});
    }else if(err?.retryable!==false) err.retryable=true; // only transient delivery/network/provider failures are retried
    if(!publicAgentLimit) await db.setIntegrationHealth('livechat',{status:'ERROR',latencyMs:Date.now()-started,error:String(err?.message||err),meta:{lastAction:'sendMessage'}}).catch(()=>{});
    await db.addDeadLetter({source:'LIVECHAT_SEND_ATTEMPT',eventKey:idempotencyKey,chatId,payload:{sourceEventId,intent,action:outboundAction,textPreview:String(text).slice(0,500)},error:String(err?.message||err),attempts:1}).catch(()=>{});
    throw err;
  }
}
...
async function buildSources(chatId,intent,normalized){
  ...
  const allRulesRows=await db.getRules(intent);
  const rulesRows=compactRulesForPrompt(allRulesRows,query,intent,20);
  const websiteProfile=await db.getSetting('website_profile',{});
  const kbRows=(await db.getRelevantKnowledge(query,intent,14)).slice(0,8);
  const promoRows=(await db.getRelevantPromoRules(query,8)).slice(0,5);
  const importantRows=(await db.getRelevantImportantInfo(query,24)).slice(0,12);
  ...
  const botResponseRows=(await getRelevantBotResponses(query,intent,14)).slice(0,7);
  const learningRows=(await db.getRelevantLearning(query,intent,10)).slice(0,5);
  const historyRows=(await db.getAutoHistoryLearning(query,intent,12)).slice(0,5);
  ...
  if(target>0){
    // Reuse the digest already stored for THIS session. New-session creation clears
    // conversation_digest/digest_message_count, so the cache cannot leak old-session context.
    // This avoids paying again to summarize the same old messages on every member turn.
    const cached=await db.getConversationDigest(chatId).catch(()=>null);
    const cachedCount=Math.max(0,Number(cached?.digest_message_count||0));
    if(cachedCount>0 && cachedCount<=target && String(cached?.conversation_digest||'').trim()){
      digest=String(cached.conversation_digest);
      digested=cachedCount;
    }
    let cursor=digested, loops=0;
    while(cursor<target && loops<30){
      ...
      try{
        ...
        await db.saveConversationDigest(chatId,digest,digested).catch(()=>{});
  ...
  const approvedCorrections=learningRows.filter(r=>String(r.source_type||'').toUpperCase()==='AI_FEEDBACK').map(r=>`[APPROVED CORRECTION ${r.intent}] Member: ${r.member_text} => Correction: ${r.correction_text||r.response_text}`).join('\n');
  const approvedLearningExamples=learningRows.filter(r=>String(r.source_type||'').toUpperCase()!=='AI_FEEDBACK').map(r=>`[APPROVED LEARNING ${r.intent}] Member: ${r.member_text} => Response: ${r.response_text}`).join('\n');
  const knowledgeOnly=[websiteKnowledge,promoKnowledge,importantKnowledge,manualKnowledge].filter(Boolean).join('\n');
  return {context,rules,knowledge:[knowledgeOnly,responses,learning].filter(Boolean).join('\n'),knowledgeOnly,responses,approvedLearningExamples,approvedCorrections,learning,hasKnowledge:Boolean(websiteKnowledge||importantKnowledge||promoKnowledge||manualKnowledge||responses||learning),knowledgeCount:kbRows.length,responseCount:botResponseRows.length,attachments,conversationDigest:digest,csStyleExamples,historyLearning,totalMessages:total,caseBrain,digestCovered:digested,digestTarget:target,importantRows,websiteProfile,recentRows};
}

async function executeConversationAgentTurn({chatId,eventId,threadId=null,text,normalized,intent,attachments=[],livechat,beforeState,src=null}){
  const sources=src||await buildSources(chatId,intent,normalized);
  const mergedAttachments=[...(sources.attachments||[]),...(attachments||[])].filter((a,i,arr)=>a?.url&&arr.findIndex(x=>x?.url===a.url)===i).slice(-3);
  ...
  const workflowForAgent=await db.getConversationWorkflow(chatId).catch(()=>({}));
  const stateForAgent=await db.getConversationState(chatId).catch(()=>beforeState||{});
  const ticketForAgent=await db.getOpenBridgeTicketByChat(chatId,stateForAgent?.session_id||null).catch(()=>null);
  const recentForAgent=sources.recentRows||[];
  const proofForAgent=latestProof(recentForAgent);
  const workflowIntent=workflowIntentFromType(workflowForAgent?.workflow_type);
  const currentIntent=String(intent||'GENERAL').toUpperCase();
  const memberIdForAgent=currentCaseUserId(text,workflowForAgent,recentForAgent,currentIntent)||extractUserId(recentForAgent)||'';
  const sessionCase=inferSessionCaseFromHistory(recentForAgent,{currentText:text,currentIntent});
  // stateForAgent is re-read AFTER any session-boundary reset. Never use the pre-boundary
  // snapshot here, otherwise last_topic from the archived session can leak into a fresh one.
  // When AI is returned after HUMAN takeover, recover the latest case from CURRENT-session
  // messages so manual CS turns remain part of the same conversation context.
  const previousIntent=String(sources.caseBrain?.primary_intent||workflowIntent||stateForAgent?.last_topic||sessionCase||'').toUpperCase();
  const meaningfulPrevious=previousIntent && !AGENT_NON_CASE_INTENTS.has(previousIntent) ? previousIntent : '';
  const meaningfulCurrent=currentIntent && !AGENT_NON_CASE_INTENTS.has(currentIntent) ? currentIntent : '';
  const activeCaseForAgent=workflowIntent||meaningfulCurrent||meaningfulPrevious||sessionCase;
  let agentState=String(workflowForAgent?.workflow_state||stateForAgent?.handling_state||'BOT_ACTIVE').toUpperCase();
  if(agentState==='AI_ACTIVE')agentState='BOT_ACTIVE';
  if(await db.isHumanTakeover(chatId))agentState='HUMAN_ACTIVE';
  const agentInput={
    sessionId:stateForAgent?.session_id||'',conversationId:stateForAgent?.conversation_id||'',livechatChatId:chatId,threadId:threadId||stateForAgent?.current_thread_id||'',
    messageIds:[eventId],rawMessages:[text],normalizedMessages:[`${normalized} [internal-risk:${actionRisk(intent)}][member-content-untrusted:true]`],
    state:agentState,currentIntent:intent,previousIntent,activeCase:activeCaseForAgent,
    memberId:memberIdForAgent,memberIdKnown:Boolean(memberIdForAgent),proofReceived:Boolean(proofForAgent),
    telegramTicketExists:Boolean(ticketForAgent),telegramTicketId:ticketForAgent?.ticket_code||ticketForAgent?.id||'',telegramTicketStatus:ticketForAgent?.status||'',
    recentMessages:recentForAgent,conversationSummary:sources.conversationDigest,knowledge:sources.knowledgeOnly,rules:sources.rules,responses:sources.responses,
    approvedLearningExamples:[sources.approvedLearningExamples,sources.historyLearning].filter(Boolean).join('\n'),approvedCorrections:sources.approvedCorrections,
    websiteProfile:sources.websiteProfile,hasKnowledge:sources.hasKnowledge,attachments:aiAttachments,caseBrain:sources.caseBrain
  };
  const started=Date.now();
  try{
    const agentDecision=await runConversationAgent({client:ai,input:agentInput,style,confidenceThreshold:config.aiConfidence});
    await db.setIntegrationHealth('openai',{status:'OK',latencyMs:Date.now()-started,meta:{lastAction:'conversationAgent',intent}}).catch(()=>{});
    return {src:sources,agentInput,agentDecision,style,mergedAttachments,aiAttachments,recentForAgent,workflowForAgent,stateForAgent,ticketForAgent,modelError:null};
  }catch(e){
    await db.setIntegrationHealth('openai',{status:'ERROR',latencyMs:Date.now()-started,error:String(e?.message||e),meta:{lastAction:'conversationAgent',intent}}).catch(()=>{});
    if(isRetryableTurnError(e)) throw e;
    await db.logAI({chatId,sourceEventId:eventId,intent,error:e.message,reason:'agent_invalid_output_safe_fallback'}).catch(()=>{});
    const agentDecision=validateAgentDecision({intent,action:'SEND_MESSAGE',confidence:0,reply:'',reason:`agent_safe_fallback:${String(e?.message||e).slice(0,180)}`,brain:sources.caseBrain},agentInput,{confidenceThreshold:config.aiConfidence});
    return {src:sources,agentInput,agentDecision,style,mergedAttachments,aiAttachments,recentForAgent,workflowForAgent,stateForAgent,ticketForAgent,modelError:e};
  }
}
...
export async function processCustomerMessage({chatId,eventId,threadId=null,text,createdAt,livechat,attachments=[],skipInsert=false,resumeReason=''}) {
  ...
  if(isGreetingTriggerMessage(text)){
    await db.insertMessage({
      chatId,eventId,threadId,senderType:'system',authorId:'system',text,
  ...
  if(!skipInsert){
    await db.insertMessage({chatId,eventId,threadId,senderType:'customer',text,normalizedText:normalized,intent,createdAt,authorId:'',attachments});
    // Message persistence and reply processing are intentionally separate. A provider/network
    // failure may happen after the customer event has already been stored. The processing
    // ledger lets a durable ingress retry resume that event instead of treating it as a
    // completed duplicate and silently losing the reply.
    const claim=await db.claimCustomerEventProcessing(chatId,eventId,{threadId,sessionId:beforeState?.session_id,staleSeconds:config.lcIngressStaleSeconds});
    if(!claim.claimed){
      if(claim.status==='DONE') return {skipped:'duplicate'};
      return {skipped:'processing_in_progress'};
    }
  ...
  try{
    const processingResult=await processingScope.run({chatId,eventId,threadId,sessionId:beforeState?.session_id||null,sendOrdinal:0},()=>db.withChatLock(chatId, async()=>{
    ...
    // Every valid customer event reaches the context-aware AI Agent before an automation/executor
    // branch is allowed to finish the turn. Deterministic workflows remain the executor/source-of-truth,
    // but semantic classification and follow-up understanding are always available to them.
    const turnAgent=await executeConversationAgentTurn({chatId,eventId,threadId,text,normalized,intent,attachments,livechat,beforeState});
    // Compatibility marker: older test doubles enter through the ai.classifyAndReply adapter inside runConversationAgent.
    const semanticIntent=String(turnAgent?.agentDecision?.intent||'').toUpperCase();
    if(semanticIntent && semanticIntent!=='GENERAL' && Number(turnAgent?.agentDecision?.confidence||0)>=0.70
      && ['GENERAL','TRANSACTION_AMBIGUOUS','UNKNOWN'].includes(String(intent||'GENERAL').toUpperCase())) intent=semanticIntent;
    ...
    // Reuse the already executed per-event AI Agent decision. This guarantees one semantic
    // decision pass per customer event and avoids a second model call on the generic path.
    const {src,agentInput,recentForAgent,mergedAttachments}=turnAgent;
    try {
      let agentDecision=turnAgent.agentDecision;
      let decision=agentDecisionToLegacy(agentDecision);
      ...
      decision=validateProDecision({decision,history:recentForAgent,brain:updatedBrain,intent,hasKnowledge:src.hasKnowledge});
      // The final validator runs AFTER all legacy safety gates, so no downstream component can
      // re-introduce NO_REPLY outside HUMAN_ACTIVE, re-ask known ID/proof, or silence low confidence.
      agentDecision=validateAgentDecision(decision,{...agentInput,caseBrain:updatedBrain},{confidenceThreshold:config.aiConfidence});
      decision=agentDecisionToLegacy(agentDecision);
      decision.reason=`agent_action:${agentDecision.action};next_state:${agentDecision.nextState};${decision.reason||''}`;
...
Silakan cek deposit member.`,requireTelegramDelivery:true});
      ...
      if(isRetryableTurnError(e)){
        await db.logError('engine','RETRYABLE_TURN_FAILURE',e.message,{chatId,eventId,intent}).catch(()=>{});
        throw e;
      }
    ...
    if(processingClaimed) await db.finishCustomerEventProcessing(chatId,eventId,{ok:true}).catch(()=>{});
    return processingResult;
  }catch(e){
    if(processingClaimed) await db.finishCustomerEventProcessing(chatId,eventId,{ok:false,error:e?.message||String(e)}).catch(()=>{});
    throw e;
  }
}
