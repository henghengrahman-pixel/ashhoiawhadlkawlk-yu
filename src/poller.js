...
export function pollerStatus(){ return {
  ...
  traffic:{polled:false,processing:false,internal:true,source:'active_chat_inventory',reason:'Browsing-only visitors have no chat_id; actionable chats stay inside this dashboard'},
...
async function processChatSnapshot(livechat,chat,summary=null,rank=null,{force=false}={}){
  ...
  const detailUpsert=await db.upsertConversation(detail,{visible:true,state:{...state,...detailState,active:detailState.active ?? state.active,lane:detailLane}});
  ...
  if(!events.length){ summaryFingerprints.set(chatId,fp);summaryLanes.set(chatId,detailLane);if(Number.isFinite(Number(rank)))summaryRanks.set(chatId,Number(rank));return {empty:1,lane:detailLane,diagnostics:livechat.chatDiagnostics(detail)}; }
...
export async function syncLaneChatForOperator(livechat,chatId,summary={}){
  ...
  const state={...livechat.chatState(detail),active:livechat.chatState(detail).active ?? true,lane};
  ...
  await db.upsertConversation(detail,{visible:true,state});
  ...
  for(const ev of events){
    ...
    if(type==='customer'){
      ...
      await ingestAgentEvent(id,ev,detail,livechat,{allowTakeover:true,allowGreetingTrigger:false}).catch(()=>{});
    }
  }
