...
test('v1.33.4 API defaults /conversations to MY_CHAT so lane lists cannot mix accidentally',()=>{
  ...
  assert.match(server,/\$7='TRAFFIC'/);
});

test('v1.33.4 lane counts are normalized and Traffic is internal aggregate',()=>{
  const server=src('../src/server.js');
  assert.match(server,/upper\(trim\(coalesce\(lc_lane,'OTHER'\)\)\) AS lc_lane/);
  assert.match(server,/laneCounts\.TRAFFIC\+=Number\(row\.count\|\|0\)/);
  assert.match(server,/traffic:\{internal:true,chatInventoryOnly:true,browsingOnlyVisitors:false\}/);
});
