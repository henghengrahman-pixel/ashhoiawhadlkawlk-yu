...
test('v1.33.4 hot poll isolates lanes and durably auto-claims queued chats in bounded batches',()=>{
...
test('v1.33.4 traffic is internal dashboard inventory without extra browsing poll',()=>{
  const src=fs.readFileSync(new URL('../src/poller.js',import.meta.url),'utf8');
  assert.match(src,/traffic:\{polled:false,processing:false,internal:true/);
  assert.match(src,/Browsing-only visitors have no chat_id/);
});
